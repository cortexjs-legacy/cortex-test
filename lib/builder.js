...
function prepare(options, callback) {
  ...
  function cb(err) {
    ...
    app.use(express.static(options.cwd));
    app.listen(options.port);
    callback(null);
  }

  var neurons = path.join(options.cwd, 'neurons');
  fs.lstat(neurons, function (err, stat) {
    if (err) {
      console.warn(
        'Warning: fs.lstat "' + neurons + '" error. \n\n'
        + 'Maybe you should run'
        + '  \x1B[1mcortex build\x1B[0m  '
        + 'first.\n'
      );
      // if err, maybe it doesn't exist
      // simply skip.
      return cb(err);
    }

    // If handle the cases cortex >= 4.3.0
    if (stat.isSymbolicLink()) {
      fs.readlink(neurons, function (err, linked) {
        if (err) {
          return cb(err);
        }

        app.use('neurons', express.static(linked));
        cb(null);
      });
      return;
    }
