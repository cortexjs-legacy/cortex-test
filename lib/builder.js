...
function generateTree(config){
  ...
  return function(done){
    ...
    shrinkwrap(config.pkg, path.join(config.cwd, "neurons"), {
      dev: true
    }, function(err, json){
      if(err){return done(err);}
      json.name = config.pkg.name;
      delete json.from;
...
function prepare(options, callback) {
  ...
  var neuron_temp_path =temp.path();

  neuronjs.write( neuron_temp_path, function(){
    app = express()
      .use(express.static(options.cwd))
      .use(express.static(path.join(__dirname, '../')))
      .use(express.static(neuron_temp_path));
